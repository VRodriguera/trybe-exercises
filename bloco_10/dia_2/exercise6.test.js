const Animals = [
  { name: 'Dorminhoco', age: 1, type: 'Dog' },
  { name: 'Binho', age: 1, type: 'Dog' },
  { name: 'Soneca', age: 2, type: 'Dog' },
  { name: 'Preguiça', age: 5, type: 'Cat' },
];

const findAnimalByName = (name) => {
  // Adicione o código aqui.
  return new Promise((resolve, reject) => {

    const animal = Animals.find((animal) => animal.name === name);
    if (animal) {

      return resolve(animal);
    }
    return reject('Nenhum animal com esse nome!');

  });
};

const getAnimal = (name) => {
  // Adicione o código aqui.
  return findAnimalByName(name).then(animal => animal);
};

const findAnimalsByAge = (age) => {
  return new Promise((resolve, reject) => {

    const animalAge = Animals.filter((animal) => animal.age === age);
    if (animalAge) {
      return resolve(animalAge);
    }
    return reject('Nenhum animal com esse nome!');

  });
};

describe('Testa animais pela idade', () => {

  it('Testa se o meu retorno é um array de objetos', () => {
    expect.assertions(1);
    return findAnimalsByAge(2).then(animals => {
      expect(animals).toEqual([{ name: 'Dorminhoco', age: 1, type: 'Dog' }, { name: 'Binho', age: 1, type: 'Dog' }])
    })
  })
})

describe('Testando promise - findAnimalByName', () => {
  describe('Quando existe o animal com o nome procurado', () => {
    test('Retorne o objeto do animal', () => {
      expect.assertions(1);
      return getAnimal('Dorminhoco').then(animal => {
        expect(animal).toEqual({ name: 'Dorminhoco', age: 1, type: 'Dog' });
      });
    });
  });

  describe('Quando não existe o animal com o nome procurado', () => {
    test('Retorna um erro', () => {
      expect.assertions(1);
      return getAnimal('Bob').catch(error =>
        expect(error).toEqual('Nenhum animal com esse nome!')
      );
    });
  });
});